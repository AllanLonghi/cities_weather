...
A WebApp consiste em quatro componentes reactjs: 
	...
	- As requisições para as APIs de cidades (C#, desenvolvida para esta aplicação) e OpenWeather (terceira) são separadas utilizando modelo de funções callBack, onde o componente delega uma função para lidar com o sucesso ou fracasso da requisição, tornando a aplicação facilmente mockavel para testes.
